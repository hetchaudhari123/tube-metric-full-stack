...
def get_youtube_id():
    try:
        ...
        if user_data:
            ...
            return jsonify({"youtubeId": user_data.get("youtubeId")}), 200
        else:
...
def get_channel_details(channel_id=None):
    if not channel_id:
...
def get_views_publishedTime():
    try:
        ...
        df_videos = df_videos.sort_values('publishedAt')
...
def get_comments(video_id, max_comments=10):
    try:
        ...
        params = {
            ...
            'maxResults': 100,  # Limit the number of comments to fetch for analysis
            'key': os.getenv('API_KEY')
        }
